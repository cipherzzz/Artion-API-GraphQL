...
func TestVideoThumbnail(t *testing.T) {
	...
	g.Expect(image.Type).To(gomega.Equal(types.MediaTypeMp4))
	thumb, err := createThumbnail(*image)
	g.Expect(err).To(gomega.BeNil())
	g.Expect(len(thumb.Data) > 10).To(gomega.BeTrue())
	g.Expect(thumb.Type).To(gomega.Equal(types.MediaTypeJpeg))
...
func TestJpgThumbnail(t *testing.T) {
	...
	g.Expect(image.Type).To(gomega.Equal(types.MediaTypePng))
	thumb, err := createThumbnail(*image)
	g.Expect(err).To(gomega.BeNil())
	g.Expect(len(thumb.Data) > 10).To(gomega.BeTrue())
	g.Expect(thumb.Type).To(gomega.Equal(types.MediaTypePng))
